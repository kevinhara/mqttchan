...
every few seconds by a quick blink (added 2026-09-19). Every character
blinks, using the exact same mechanism for all 99: `Portrait::blink`, a
frame `firmware/tools/gen_portraits.py` synthesizes for every character
(`_SYNTHETIC_BLINKS`, one dict entry per character) by closing an eye region
hand-picked against that character's own art and cutting a thin line back
through it - kept entirely separate from the pack's own `frames`/
`frameCount`, which are unchanged and still only drive the talk-time mouth
flap (`advanceTalkFrame()`).

**Correction, same day:** two earlier passes at this got it wrong before
landing here. First pass gave the two single-frame characters (44/portrait
112, 66/portrait 156) a head-nod instead of a blink, since they had no
second frame to flash to - not what was asked for. Second pass "fixed" that
by synthesizing a real second frame for just those two, but left every other
character blinking by flashing to its own frame 1 - which, on a live board,
turned out to be a mouth-open frame for a large fraction of the pack, so
idle was visibly opening its mouth instead of blinking. Every character now
gets a synthesized `blink` frame, not just the two that had no frame 1 to
reuse in the first place - the reuse was the bug, not just the exception
list around it. Cycles through that character's own 1-4 talk frames while a
message types, and a new random character is picked each time an
announcement finishes. Build-verified and QA'd via a rendered contact sheet
of all 99 blink frames against their originals - not yet looked at on the
