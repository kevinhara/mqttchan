...
// (startBlink()). Every character has one, so every character blinks - most
// (97 of 99) ship it in the pack's own art already (usually a mouth-flap
// frame doing double duty, since the pack wasn't authored with a dedicated
// eye-closed frame - the closest thing to a blink available without new
// art); the two that shipped with only one frame (index 44 and 66 - see
// portraits_data.h) get a synthetic second frame instead, generated by
// firmware/tools/gen_portraits.py (_SYNTHETIC_BLINKS) rather than excluded.
// While a message is being typed, MqttLink drives it through the character's
// own frame set instead (advanceTalkFrame()) - most characters ship 2-4
// frames (a blink or a mouth flap), so this is a real per-character
// animation, not the sine-noise mouth-ratio placeholder the m5avatar version
// faked. MqttLink
...
class PortraitFace {
 ...
 private:
  ...
  // it is - see the file header). Every character has a frame 1 to flash to
  // (portraits_data.h, including the two synthesized ones), so this needs no
  // per-character fallback. tick() holds it for kBlinkHoldMs before resuming
  // the breathing sway.
  void startBlink(uint32_t nowMs) {
    blinking_ = true;
    blinkStartMs_ = nowMs;
    blinkHoldMs_ = kBlinkHoldMs;
    draw(1, (lastOffset_ == kNoOffset) ? 0 : lastOffset_);
  }
